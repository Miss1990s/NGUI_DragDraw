...
#region Fill Functions
    ...
    protected virtual Vector3 CreateVertexPos(float x, float y, float z=0f)
    {
	...
	void SimpleFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		verts.Add(CreateVertexPos(v.x, v.y));
		verts.Add(CreateVertexPos(v.x, v.w));
		verts.Add(CreateVertexPos(v.z, v.w));
		verts.Add(CreateVertexPos(v.z, v.y));
	...
	void SlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		for (int x = 0; x < 3; ++x)
		{
			...
			for (int y = 0; y < 3; ++y)
			{
				...
				verts.Add(CreateVertexPos(mTempPos[x].x, mTempPos[y].y));
				verts.Add(CreateVertexPos(mTempPos[x].x, mTempPos[y2].y));
				verts.Add(CreateVertexPos(mTempPos[x2].x, mTempPos[y2].y));
				verts.Add(CreateVertexPos(mTempPos[x2].x, mTempPos[y].y));
	...
	void TiledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		while (y0 < v.w)
		{
			...
			while (x0 < v.z)
			{
				...
				verts.Add(CreateVertexPos(x0, y0));
				verts.Add(CreateVertexPos(x0, y1));
				verts.Add(CreateVertexPos(x1, y1));
				verts.Add(CreateVertexPos(x1, y0));
